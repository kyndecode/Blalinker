...
export default function Footer() {
  ...
  const navigationLinks = [
    { label: t('nav.home'), to: '/' },
    { label: t('footer.nav_search'), to: '/search' },
    { label: t('nav.become_provider'), to: '/register?role=provider' },
    { label: t('nav.about'), to: '/about' },
    { label: t('nav.contact'), to: '/contact' },
    { label: t('footer.nav_faq'), to: '/help' },
  ];
    ...
    <footer className="bg-[#1a2744] text-gray-400">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
          <div className="space-y-4">
            ...
            <p className="text-sm text-white/50 font-medium">{t('footer.brand_subtitle')}</p>
            <p className="text-sm text-white/40 leading-relaxed">{t('footer.brand_description')}</p>
            <div>
          ...
          <div>
            ...
            <div className="mt-8">
              ...
              <div className="flex flex-wrap gap-1.5">
                {['Wave', 'Orange Money', 'Free Money', 'Stripe'].map((payment) => (
                  <span key={payment} className="px-2 py-0.5 bg-white/10 rounded text-xs text-white/50 border border-white/10">
                    {payment}
                  </span>
                ))}
              </div>
          ...
          <div>
            ...
            <ul className="space-y-4">
              <li>
                <p className="text-xs text-white/40 mb-0.5">{t('footer.email_label')}</p>
      ...
      <div className="border-t border-white/10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex flex-col sm:flex-row items-center justify-between gap-2">
          ...
          <p className="text-xs text-white/30">{t('footer.made_for_africa')}</p>
        </div>
      </div>
    </footer>
  );
