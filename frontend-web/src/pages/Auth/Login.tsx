...
export default function Login() {
  ...
  const [mfaState, setMfaState] = useState<{ required: boolean; tempToken: string }>({ required: false, tempToken: '' });
    ...
    return (
      <div className={PAGE}>
        <div className={CARD}>
          ...
          {apiError && <p className="text-sm text-red-600 dark:text-red-400 mb-4 bg-red-50 dark:bg-red-900/20 rounded-xl p-3" role="alert">{apiError}</p>}
          <form onSubmit={handleMfaSubmit} className="flex flex-col gap-4">
            <Input label="Code OTP" value={otpCode} onChange={(e) => setOtpCode(e.target.value)}
              placeholder={t('auth.otp_placeholder')} inputMode="numeric" maxLength={6} required autoFocus />
  ...
  return (
    <div className={PAGE}>
      <div className={CARD}>
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1 mb-6">Accédez à votre espace BLA Services</p>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4" noValidate>
          <Input label="Téléphone ou Email" placeholder="+221 77 000 00 00 ou email@exemple.com"
            error={errors.identifier?.message} autoComplete="username" {...register('identifier')} />
          <div>
          ...
          <Button type="submit" loading={isSubmitting} className="w-full">{t('auth.login')}</Button>
        </form>
